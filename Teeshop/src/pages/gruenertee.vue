      ...
      <q-scroll-area style="height:843px;width: 100%; background-color: #EEEEEE">
        <div class="q-pa-md">
          <div class="q-col-gutter-md row items-start">
            <div v-for="(produkt, index) in gruentee" :key="index" class="col-6">
    ...
    <q-dialog v-model="showArticle">
      <q-card style="width: 600px; max-width: 800px; background-color: #dddddd;">
        <q-item style="background-color: #dddddd; width:600px; max-width: 800px; height:330px; margin:50px 0 50px 0">
          <div v-if="gruentee[indexNumber].id <= 4" class="absolute-bottom-right text-subtitle1 text-center"><br>
          ...
          <q-item-section>
            <q-img
              :src="gruentee[indexNumber].imagename"
              style="height: 100%; width: 250px; margin: 10px"
            >
              <div v-if="gruentee[indexNumber].id <= 4" class="absolute-bottom text-subtitle1 text-center">
                BELIEBT
              </div>
            </q-img>
          </q-item-section>
          <q-item-section style="font-size: 16px"><b>{{gruentee[indexNumber].name}}</b><br>{{gruentee[indexNumber].beschreibung}}
            <br><br>
            ...
            <b><span style="text-align: left">Preis: {{ (inhaltDialog / 100 * this.gruentee[this.indexNumber].preis * amount).toFixed(2) }}€</span></b><br>
          </q-item-section>
        </q-item>
      </q-card>
    </q-dialog>
  ...
  methods: {
    ...
    filterGruentee () {
      for (let i = 0; i < this.alleProdukte.length; i++) {
        if (this.alleProdukte[i].TeeKategorie === 'gruentee') {
          this.gruentee.push(this.alleProdukte[i])
        }
        console.log(this.gruentee)
      }
    },
    addToCart (index) {
