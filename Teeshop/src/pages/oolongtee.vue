      ...
      <q-scroll-area style="height:843px;width: 100%; background-color: #EEEEEE">
        <div class="q-pa-md">
          <div class="q-col-gutter-md row items-start">
            <div v-for="(produkt, index) in oolongtee" :key="index" class="col-6">
    ...
    <q-dialog v-model="showArticle">
      <q-card style="width: 600px; max-width: 800px; background-color: #dddddd;">
        <q-item style="background-color: #dddddd; width:600px; max-width: 800px; height:330px; margin:50px 0 50px 0">
          <div v-if="oolongtee[indexNumber].id <= 4" class="absolute-bottom-right text-subtitle1 text-center"><br>
          ...
          <q-item-section>
            <q-img
              :src="oolongtee[indexNumber].imagename"
              style="height: 100%; width: 250px; margin: 10px"
            >
              <div v-if="oolongtee[indexNumber].id <= 4" class="absolute-bottom text-subtitle1 text-center">
                BELIEBT
              </div>
            </q-img>
          </q-item-section>
          <q-item-section style="font-size: 16px"><b>{{oolongtee[indexNumber].name}}</b><br>{{oolongtee[indexNumber].beschreibung}}
            <br><br>
            ...
            <b><span style="text-align: left">Preis: {{ (inhaltDialog / 100 * this.oolongtee[this.indexNumber].preis * amount).toFixed(2) }}€</span></b><br>
          </q-item-section>
        </q-item>
      </q-card>
    </q-dialog>
...
export default {
  ...
  methods: {
    ...
    test () {
      axios.get('http://127.0.0.1:8000/api/test').then(response => { this.name = response.data }).then(response => this.filterOolongtee())
    },
    filterOolongtee () {
      for (let i = 0; i < this.alleProdukte.length; i++) {
        if (this.alleProdukte[i].TeeKategorie === 'oolongtee') {
          this.oolongtee.push(this.alleProdukte[i])
        }
        console.log(this.oolongtee)
      }
    },
    addToCart (index) {
